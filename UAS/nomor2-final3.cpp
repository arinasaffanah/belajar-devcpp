...
void gotoxy(int x, int y)
{
	...
	//	Membuat program TST (Tutorial Service Time) yaitu layanan konsul PR, tugas sekolah, menyelesaikan soal-soal sulit, dan lainnya secara gratis.
	
	
	//deklarasi global
	string ruangan[1];
	string nama[1], hari[1], no[1], kelas[1], oHari[1], oMapel[1], oJam[1];
	string mapel1[4] = {"Matematika","Fisika","Kimia","Biologi"};
	string mapel2[4] = {"Matematika","Fisika","Kimia","Biologi"};
	string mapel3[4] = {"Matematika","Fisika ","Kimia  ","Biologi"};
	string mapel4[4] = {"Matematika","Fisika","Kimia  ","Biologi"};

	string jam[4] = {"9-11","10-12","13-15","16-18"};
	string jwb[1], jwbHari[1], jwbMapel[1];
	int pilHari[1], pilMapel[1], pilHariLagi[1];
	string tanggal[1];
...
void jadwal(){
		...
		gotoxy(65,6);cout<<"RABU";
		gotoxy(100,6);cout<<"KAMIS";
		...
		for(int i=0 ; i<4; i++){
			gotoxy(60,8+i);cout<<i+1<<")";
			gotoxy(65,8+i);cout<<mapel3[i]<<"\t"<< setw (1) << "("  << jam[i] << ")";
		}
		
		for(int i=0 ; i<4; i++){
			gotoxy(95,8+i);cout<<i+1<<")";
			gotoxy(100,8+i);cout<<mapel4[i]<<"\t"<< setw (1) << "("  << jam[i] << ")";
		}
